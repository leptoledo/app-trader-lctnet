                    ...
                    <div className="bg-white dark:bg-slate-900/40 backdrop-blur-xl p-16 rounded-[3rem] border border-slate-200/60 dark:border-slate-800/60 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-8">
                            ...
                            <div className="space-y-3">
                                <p className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.4em]">Recurso Exclusivo</p>
                                <h1 className="text-4xl md:text-5xl font-heading font-black text-slate-900 dark:text-white tracking-tight">Importação em Massa</h1>
                            </div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400 font-bold text-lg max-w-lg mx-auto leading-relaxed">
                                ...
                                <span className="block mt-2 text-blue-600 dark:text-blue-400 font-black">Funcionalidade exclusiva do plano PRO.</span>
                            </p>

                            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                                <Button size="lg" className="bg-blue-600 hover:bg-blue-700 rounded-2xl h-16 px-12 font-black uppercase tracking-tighter text-lg shadow-xl shadow-blue-500/20 w-full sm:w-auto" asChild>
                                    <Link href="/pricing" className="flex items-center gap-2">
                                        <Sparkles className="h-5 w-5" /> Fazer Upgrade Hoje
                                    </Link>
                                </Button>
                                <Button variant="ghost" className="rounded-2xl h-16 px-8 font-bold text-slate-400 hover:text-slate-900 dark:hover:text-white dark:hover:bg-slate-800/50 w-full sm:w-auto" asChild>
                            ...
                            <div className="pt-8 grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
                                {[
                                    ...
                                    <div key={i} className="bg-slate-50/50 dark:bg-slate-950/30 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/50">
                                        ...
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">{feature.desc}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <div className="mx-auto grid w-full max-w-7xl gap-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.3em] mb-1">Automação</p>
                            <h1 className="text-3xl font-heading font-bold text-slate-900 dark:text-white tracking-tight">Sync Histórico</h1>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="bg-white dark:bg-slate-900/40 backdrop-blur-md p-10 rounded-[2.5rem] border border-slate-200/60 dark:border-slate-800/60 shadow-xl space-y-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">1. Fonte de Dados</label>
                                    <Select value={selectedAdapter} onValueChange={onAdapterChange}>
                                        <SelectTrigger className="h-14 rounded-2xl border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 font-black text-sm transition-all focus:ring-blue-500">
                                        ...
                                        <SelectContent className="rounded-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
                                            {BROKER_ADAPTERS.map(adapter => (
                                                <SelectItem key={adapter.id} value={adapter.id} className="font-bold py-3">{adapter.name}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">2. Conta de Destino</label>
                                    <Select value={selectedAccount} onValueChange={setSelectedAccount}>
                                        ...
                                        <SelectContent className="rounded-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
                                            {accounts.map(acc => (
                                                <SelectItem key={acc.id} value={acc.id} className="font-bold py-3">{acc.name} ({acc.currency})</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                            ...
                            <div className="relative group">
                                ...
                                >
                                    <div className="space-y-4 text-center p-8">
                                        ...
                                        <div className="space-y-1">
                                            <p className="font-black text-lg text-slate-900 dark:text-white uppercase tracking-tight">Arraste seu arquivo CSV</p>
                                ...
                                {file && (
                                    <div className="absolute top-4 right-4 animate-in fade-in zoom-in-50 duration-300">
                                        <Button variant="ghost" size="sm" onClick={() => { setFile(null); setPreviewCount(0); setPreviewTrades([]); }} className="h-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-red-500 font-black text-xs hover:bg-red-50">
                                            REMOVER
                                        </Button>
                                    </div>
                                )}
                        ...
                        <Card className="rounded-[2.5rem] border-none shadow-2xl bg-slate-900 dark:bg-slate-950 dark:border dark:border-slate-800/80 text-white h-[660px] flex flex-col overflow-hidden relative">
                            ...
                            <div className="p-10 flex-1 flex flex-col relative z-10 space-y-8 h-full">
                                <div className="flex items-center justify-between">
                                    <h2 className="text-xl font-heading font-black uppercase tracking-tight">Prévia dos Dados</h2>
                                    <div className="flex items-center gap-2">
                                        <div className={cn(
                                            "w-2 h-2 rounded-full",
                                            previewCount > 0 ? "bg-emerald-500 animate-pulse" : "bg-slate-700"
                                        )} />
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Live Validator</span>
                                    </div>
                                </div>
                                ...
                                {previewCount > 0 ? (
                                    <div className="flex-1 flex flex-col min-h-0 space-y-8">
                                        ...
                                        <div className="bg-white/5 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 flex items-center justify-between">
                                            <div>
                                                <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mb-1">Total Detectado</p>
                                                <p className="text-4xl font-heading font-black tracking-tighter">{previewCount} trades</p>
                                            </div>
                                        ...
                                        <div className="space-y-4 flex-1 flex flex-col min-h-0">
                                            <div className="flex items-center justify-between px-2">
                                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Últimas Entradas</p>
                                                <p className="text-[10px] font-bold text-slate-600">Mostrando top 5</p>
                                            </div>
                                                ...
                                                {previewTrades.slice(0, 5).map((t, i) => (
                                                    <div key={i} className="bg-white/[0.03] border border-white/5 p-4 rounded-2xl flex justify-between items-center transition-all hover:bg-white/[0.06]">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <p className="font-black text-sm text-white tracking-tight">{t.symbol}</p>
                                                                <p className="text-[10px] text-slate-500 font-bold uppercase">{new Date(t.entry_date).toLocaleDateString()}</p>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            <p className="text-white font-black text-sm tracking-tight">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(t.pnl_net || 0)}</p>
                                                            <p className="text-[10px] text-slate-500 font-bold font-mono">@{t.entry_price}</p>
                                                        </div>
                                                    </div>
                                                ))}
                                                {previewCount > 5 && (
                                                    <div className="py-4 text-center">
                                                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">+ {previewCount - 5} outros registros...</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-blue-500/10 rounded-2xl p-4 border border-blue-500/20 flex items-start gap-3">
                                            ...
                                            <p className="text-[10px] text-blue-400/80 font-bold leading-relaxed uppercase tracking-wider">
                                                Validação pronta. Ao processar, todos os trades serão vinculedos à conta <span className="text-blue-300 underline font-black">{accounts.find(a => a.id === selectedAccount)?.name || 'selecionada'}</span>.
                                            </p>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex-1 flex flex-col items-center justify-center space-y-4 opacity-30">
                                        ...
                                        <p className="text-xs font-black text-slate-600 uppercase tracking-widest">Aguardando arquivo...</p>
                                    </div>
                                )}

                                <Button
                                    className="w-full h-20 rounded-[2rem] bg-blue-600 hover:bg-blue-700 text-white font-black text-lg shadow-2xl shadow-blue-500/20 transition-all hover:-translate-y-1 active:scale-95 disabled:opacity-50 disabled:translate-y-0"
